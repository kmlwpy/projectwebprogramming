...
continueBtn.addEventListener("click", () => {
    ...
    total_correct.innerHTML = `ถูก ${correct = 0} ข้อ ใน ${MCQS.length} คำถาม`;
});
...
next_question.addEventListener("click", () => {
    ...
    if (index !== MCQS.length - 1) {
        ...
        total_correct.innerHTML = `ถูก ${correct} ข้อ ใน ${MCQS.length} คำถาม`;
        clearInterval(interval);
        interval = setInterval(countDown, 1000);
    } else {
        index = 0;
        ...
        points.innerHTML = `คุณตอบถูก ${correct} ข้อ`;
